...
namespace Planned.Data
{
	public partial class DataControllerBase
    {
        ...
        public static ControllerConfiguration CreateConfigurationInstance(Type t, string controller)
        {
            ...
            if (config == null)
            {
                ...
                if (res == DefaultDataControllerStream)
                	res = null;
                if (res == null)
                	res = t.Assembly.GetManifestResourceStream(String.Format("Planned.Controllers.{0}.xml", controller));
                if (res == null)
                	res = t.Assembly.GetManifestResourceStream(String.Format("Planned.{0}.xml", controller));
                if (res == null)
                {
        ...
        public static string CreateReportInstance(Type t, string name, string controller, string view, bool validate)
        {
            ...
            if (t == null)
            	t = typeof(Planned.Data.Controller);
            Stream res = t.Assembly.GetManifestResourceStream(String.Format("Planned.Reports.{0}", name));
            if (res == null)
            	res = t.Assembly.GetManifestResourceStream(String.Format("Planned.{0}", name));
            if (res == null)
            {
            ...
            if (isGeneric)
            {
                ...
                ControllerConfiguration config = Planned.Data.Controller.CreateConfigurationInstance(t, controller);
