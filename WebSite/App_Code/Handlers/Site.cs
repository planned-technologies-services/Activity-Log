...
namespace Planned.Handlers
{
    ...
    public class SiteBase : Planned.Web.PageBase
    {
        ...
        protected override void OnInit(EventArgs e)
        {
            ...
            if (_isTouchUI)
            {
                if (_pageFooterContent != null)
                	_pageFooterContent.Text = "<footer style=\"display:none\"><small>&copy; 2017 Planned. ^Copyright^All rights re" +
        ...
        protected virtual void InitializeSiteMaster()
        {
            ...
            string themePath = Server.MapPath("~/App_Themes/Planned");
            if (Directory.Exists(themePath))
            	foreach (string stylesheetFileName in Directory.GetFiles(themePath, "*.css"))
                {
                    ...
                    if (!(fileName.Equals("_Theme_Aquarium.css")))
                    {
                        ...
                        link.Href = ("~/App_Themes/Planned/" + fileName);
